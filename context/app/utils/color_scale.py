...
class _ColorScale():
    def __init__(self, *hex_list):
        self.rgb_list = [_hex_to_rgb(h) for h in hex_list]

    def log(self, min, max):
        points = int(log10(max) - log10(min)) + 2
        interpolated = n_colors(
            unlabel_rgb(self.rgb_list[1]),
            unlabel_rgb(self.rgb_list[0]),
            points)
        missing_zero = [
    ...
    def linear(self):
        return [[i, color] for (i, color) in enumerate(self.rgb_list)]
